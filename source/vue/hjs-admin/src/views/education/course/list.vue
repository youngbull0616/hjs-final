    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300px" label="操作" align="center">
        <template slot-scope="{row}">
          ...
          <router-link :to="{path:'/education/course/upload', query:{id:row.id}}" class="link-left">
            <el-button size="mini">上传课件</el-button>
          </router-link>
          <el-button   size="mini" type="danger" @click="delCourse(row)" class="link-left">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    search () {
      ...
      courseApi.pageList(this.queryParam).then(data => {
    ...
    delCourse (row) {
      let _this = this
      courseApi.deleteSubject(row.id).then(re => {
        if (re.code === 1) {
          _this.search()
